...
class RefSegRSDataset(Dataset):
    def __init__(self, base_data_path, tokenizer, image_size = 1024):
        ...
        self.RefSegRS_images_root = os.path.join(self.base_data_path, "train/img")
        self.RefSegRS_labels_root = os.path.join(self.base_data_path, "train/lbl")
        self.RefSegRS_txt = os.path.join(self.base_data_path, "train/json")       
        self.images = self.load_file_paths(self.RefSegRS_images_root, valid_extensions=('.jpg', '.jpeg', '.png'))
        self.masks = self.load_file_paths(self.RefSegRS_labels_root , valid_extensions=('.png',))
    ...
    def __getitem__(self, idx):
        ...
        prefix_inst = 'This is an image <image>, Please doing Segmentation according to the following instruction:'
...
class ReasonSegDataset(Dataset):
    def __init__(self, base_data_path, tokenizer, image_size = 1024):
        ...
        self.tokenizer = tokenizer
        self.image_size = image_size
        self.ReasonSeg_images_root = os.path.join(base_data_path, "train/images")
        self.ReasonSeg_labels_root = os.path.join(base_data_path, "train/labels")
        self.ReasonSeg_QAs_root = os.path.join(base_data_path, "train/QAs")
